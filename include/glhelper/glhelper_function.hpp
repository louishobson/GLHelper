 ...
 * as a result, only 1, 2, or 3 variable-input functions can be used, since there are only 1D, 2D and 3D textures
 ...
 * 
 * struct function_struct_1D/2D/3D
 * {
 *     sampler1D/2D/3D outputs;
 *     
 *     float lower_domains [0/1/2];
 *     float upper_domains [0/1/2];
 * }
 * 
 * outputs: a sampler for the outputs of the function, either 1, 2, or 3D depending on the dimension of ths struct
 * lower_domains: an array of size dependant on the dimension of the function, which stores the value that a texcoord dimension of 0 should equate to
 * upper_domains: an array of size dependant on the dimension of the function, which stores the value that a texcoord dimension of 1 should equate to
 * 
 * hence, the texcoord values [0-1] will be linearly interpolated to [lower_domain-upper_domain] for each dimension
 * 
 *
 * 
...
namespace glh
{
    namespace math
    {
         ...
         *
         * Rt: the function's return type
         * Ps: the parameter types of the function
         */
        template<class Rt, class Fg, class ...Ps> class glsl_function;
    }
